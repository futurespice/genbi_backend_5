...
async def register(
    ...
    ⚠️ ВАЖНО: Роль всегда 'client'
    Для получения роли 'company' нужно:
    1. Зарегистрироваться
    2. Подать заявку через POST /api/v1/applications
    3. Дождаться одобрения администратором
    """
    ...
    # ✅ ИСПРАВЛЕНО: Создание пользователя - роль всегда 'client'
    user = User(
        email=user_in.email,
...
async def check_auth(
    ...
    Используется на фронтенде для редиректа:
    - Если авторизован → перенаправить с /login на /dashboard
    - Если не авторизован → показать форму логина
    """
    return {
        "authenticated": True,
